...
node {
    ...
    /* In order to execute our tests, we need a running PostgreSQL instance.
     * Instead of expecting our Jenkins agents to have PostgreSQL running and
     ...
     */
    docker.image('postgres').withRun('-p 5432:5432 --name=postgres') { c ->
         ...
         * container
         */
        docker.image('rtyler/rvm:2.3.0').inside('--link=postgres:postgres') {
